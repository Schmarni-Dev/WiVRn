...
wivrn::video_encoder_vulkan_h264::video_encoder_vulkan_h264(wivrn_vk_bundle & vk, vk::Rect2D rect, vk::VideoEncodeCapabilitiesKHR encode_caps, float fps, uint8_t stream_idx, uint8_t image_layer, uint64_t bitrate) :
        video_encoder_vulkan(vk, rect, encode_caps, fps, stream_idx, image_layer, bitrate),
        sps{
                .flags =
                        {
        ...
        uint8_t image_layer)
{
	...
	std::unique_ptr<video_encoder_vulkan_h264> self(new video_encoder_vulkan_h264(vk, rect, encode_caps, fps, stream_idx, image_layer, settings.bitrate));
